...
public class BlockModelGenerators {
   ...
   private void createSuspiciousSandBlock() {
      this.blockStateOutput.accept(MultiVariantGenerator.multiVariant(Blocks.SUSPICIOUS_SAND).with(PropertyDispatch.property(BlockStateProperties.DUSTED).generate((p_272377_) -> {
         String s = "_" + p_272377_;
         ResourceLocation resourcelocation = TextureMapping.getBlockTexture(Blocks.SUSPICIOUS_SAND, s);
         return Variant.variant().with(VariantProperties.MODEL, ModelTemplates.CUBE_ALL.createWithSuffix(Blocks.SUSPICIOUS_SAND, s, (new TextureMapping()).put(TextureSlot.ALL, resourcelocation), this.modelOutput));
      })));
