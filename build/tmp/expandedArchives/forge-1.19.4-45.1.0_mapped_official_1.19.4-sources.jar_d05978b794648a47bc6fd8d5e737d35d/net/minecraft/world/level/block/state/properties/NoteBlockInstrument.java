...
public enum NoteBlockInstrument implements StringRepresentable {
   ...
   public static Optional<NoteBlockInstrument> byStateAbove(BlockState p_261556_) {
      if (p_261556_.is(Blocks.ZOMBIE_HEAD)) {
         return Optional.of(ZOMBIE);
      } else if (p_261556_.is(Blocks.SKELETON_SKULL)) {
         return Optional.of(SKELETON);
      } else if (p_261556_.is(Blocks.CREEPER_HEAD)) {
         return Optional.of(CREEPER);
      } else if (p_261556_.is(Blocks.DRAGON_HEAD)) {
         return Optional.of(DRAGON);
      } else if (p_261556_.is(Blocks.WITHER_SKELETON_SKULL)) {
         return Optional.of(WITHER_SKELETON);
      } else if (p_261556_.is(Blocks.PIGLIN_HEAD)) {
         return Optional.of(PIGLIN);
      } else {
         return p_261556_.is(Blocks.PLAYER_HEAD) ? Optional.of(CUSTOM_HEAD) : Optional.empty();
      }
   }

   public static NoteBlockInstrument byStateBelow(BlockState p_61667_) {
