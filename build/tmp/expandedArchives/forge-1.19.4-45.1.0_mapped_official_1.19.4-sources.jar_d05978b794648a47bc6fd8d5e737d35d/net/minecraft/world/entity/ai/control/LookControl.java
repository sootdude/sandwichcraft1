...
public class LookControl implements Control {
   ...
   public void tick() {
      ...
      if (this.lookAtCooldown > 0) {
         --this.lookAtCooldown;
         this.getYRotD().ifPresent((p_275069_) -> {
            this.mob.yHeadRot = this.rotateTowards(this.mob.yHeadRot, p_275069_, this.yMaxRotSpeed);
         });
         this.getXRotD().ifPresent((p_275965_) -> {
            this.mob.setXRot(this.rotateTowards(this.mob.getXRot(), p_275965_, this.xMaxRotAngle));
         });
      } else {
