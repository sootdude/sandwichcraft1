   ...
   public void render(PoseStack p_94643_) {
      ...
      if (this.isListening && !this.subtitles.isEmpty()) {
         ...
         for(SubtitleOverlay.Subtitle subtitleoverlay$subtitle1 : this.subtitles) {
            ...
            p_94643_.pushPose();
            p_94643_.translate((float)this.minecraft.getWindow().getGuiScaledWidth() - (float)l * 1.0F - 2.0F, (float)(this.minecraft.getWindow().getGuiScaledHeight() - 35) - (float)(i * (i1 + 1)) * 1.0F, 0.0F);
            p_94643_.scale(1.0F, 1.0F, 1.0F);
            fill(p_94643_, -l - 1, -j1 - 1, l + 1, j1 + 1, this.minecraft.options.getBackgroundColor(0.8F));
            int j2 = i2 + -16777216;
            if (!flag) {
               if (d1 > 0.0D) {
                  drawString(p_94643_, this.minecraft.font, ">", l - this.minecraft.font.width(">"), -j1, j2);
               } else if (d1 < 0.0D) {
                  drawString(p_94643_, this.minecraft.font, "<", -l, -j1, j2);
               }
            }

            drawString(p_94643_, this.minecraft.font, component, -k1 / 2, -j1, j2);
            p_94643_.popPose();
            ++i;
         }
