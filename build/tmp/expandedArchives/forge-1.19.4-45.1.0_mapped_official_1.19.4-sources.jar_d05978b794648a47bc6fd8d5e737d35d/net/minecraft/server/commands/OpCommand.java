...
public class OpCommand {
   ...
   public static void register(CommandDispatcher<CommandSourceStack> p_138080_) {
      p_138080_.register(Commands.literal("op").requires((p_138087_) -> {
         ...
         return SharedSuggestionProvider.suggest(playerlist.getPlayers().stream().filter((p_275901_) -> {
            return !playerlist.isOp(p_275901_.getGameProfile());
         }).map((p_275902_) -> {
            return p_275902_.getGameProfile().getName();
         }), p_138085_);
      }).executes((p_138082_) -> {
